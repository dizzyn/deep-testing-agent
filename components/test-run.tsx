...
import type { TesterAgentUIMessage } from "@/agents/tester";
...
export function TestRun({ sessionData: { testBrief } }: ChatProps) {
  const [hasLoadedHistory, setHasLoadedHistory] = useState(false);
  const [inputValue, setInputValue] = useState("");

  if (!testBrief) {
    throw new Error("No test brief provided");
  }

  const { messages, sendMessage, setMessages } = useChat<TesterAgentUIMessage>({
    messages: [
      {
        role: "assistant",
        id: "brief",
        parts: [{ text: testBrief, type: "text" }],
      },
    ],
  });
